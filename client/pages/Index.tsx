...
export default function Index() {
  ...
  const miningAmount = useMemo(
    () => adminConfig.miningAmount,
    [adminConfig.miningAmount],
  );
  ...
  const handleDailyBonus = () => {
    ...
    const newBalance = parseFloat(
      (miningBalance + adminConfig.dailyBonus).toFixed(8),
    );
    ...
    <div className="min-h-screen pb-20 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 px-4 pt-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-xs mt-2">
            ≈ {(miningBalance * 40000).toFixed(2)} USDT
          </p>
        </div>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
        <p className="text-blue-900 text-sm">
          💡 <strong>Tip:</strong> Refer friends to earn more! Every referral
          gets you {adminConfig.referralProfit}% of their mining.
        </p>
      </div>
    </div>
  );
