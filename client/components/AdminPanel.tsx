...
import { X, Save, Settings, DollarSign, ListTodo, Users, CreditCard } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
  const { adminConfig, updateAdminConfig, tasks, addTask, removeTask, updateTask, withdrawals, updateWithdrawalStatus, allUsers, updateUser, blockUser } = useAppContext();
  
  const [activeTab, setActiveTab] = useState<'finance' | 'tasks' | 'withdrawals' | 'users'>('finance');
  const [localConfig, setLocalConfig] = useState(adminConfig);
  const [newTask, setNewTask] = useState({ title: '', description: '', type: 'adsgram' as const, reward: 0 });
        ...
        <div className="p-4 space-y-4">
          ...
          {activeTab === 'finance' && (
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-gray-900">Finance Settings</h3>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, miningAmount: Number(e.target.value) })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, dailyBonus: Number(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, minWithdrawal: Number(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, referralProfit: Number(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, telegramBotId: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, telegramUserId: e.target.value })}
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e => setLocalConfig({ ...localConfig, adsgarmCode: e.target.value })}
          ...
          {activeTab === 'tasks' && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {tasks.map(task => (
                  <div key={task.id} className="bg-gray-50 rounded-lg p-3 flex justify-between items-start">
                    <div className="flex-1">
                      <p className="font-semibold text-gray-900 text-sm">{task.title}</p>
                      <p className="text-gray-600 text-xs">{task.description}</p>
                      <p className="text-blue-600 text-xs mt-1">Reward: {task.reward.toFixed(8)}</p>
                    </div>
                    <button
          ...
          {activeTab === 'withdrawals' && (
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-gray-900">Withdrawal Management</h3>

              <div className="space-y-3">
                {withdrawals.length === 0 ? (
                  <p className="text-gray-600 text-center py-4">No withdrawals</p>
                ) : (
                  withdrawals.map(withdrawal => (
                    <div key={withdrawal.id} className="bg-gray-50 rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          <p className="font-mono text-sm text-gray-900 break-all">{withdrawal.address}</p>
                          <p className="text-gray-600 text-xs">{withdrawal.amount.toFixed(8)}</p>
                        </div>
                        <select
                      ...
                      <p className="text-gray-500 text-xs">{new Date(withdrawal.createdAt).toLocaleString()}</p>
                    </div>
                  ))
                )}
              </div>
                ...
                {allUsers.map(user => (
                  ...
                  >
                    <div className="flex justify-between items-center mb-2">
                      <p className="font-semibold text-gray-900">User {user.id}</p>
                      <span className={`text-xs px-2 py-1 rounded ${user.isBlocked ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                        {user.isBlocked ? 'Blocked' : 'Active'}
                      </span>
                    </div>
                    <p className="text-gray-600 text-sm">Balance: {user.balance.toFixed(8)}</p>
                      ...
                      <div className="mt-3 space-y-2 pt-3 border-t border-gray-200">
                        <button
                          onClick={e => {
                            ...
                            if (newBalance) {
                              updateUser(user.id, { balance: Number(newBalance) });
                            }
                          }}
